...
export const batchUpdateEventsTool = {
...
export async function handleBatchUpdateEvents(args, calendarService) {
    ...
    for (let i = 0; i < input.updates.length; i += options.batchSize) {
        ...
        const batchPromises = batch.map(async (updateData, index) => {
            try {
                const event = await withRetry(() => calendarService.updateEvent(updateData), retryOptions);
                results.successful++;
...
//# sourceMappingURL=batch-update-events.js.map